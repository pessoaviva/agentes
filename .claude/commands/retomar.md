...
(O comando se chama `/retomar` — e não `/status` — para não colidir com o `/status`
nativo do Claude Code, que mostra versão/modelo/conta.)
...
> `/clear` (ou `/compact`) sem medo — o `ESTADO.md` guarda tudo — e usar `/retomar` para
> voltar em segundos com a janela zerada. É assim que projeto longo fica barato.
...
4. **Não atualize o `ESTADO.md` neste comando** — /retomar é só leitura. Se o usuário
